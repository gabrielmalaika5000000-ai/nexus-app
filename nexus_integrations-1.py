...
class GmailClient:
    ...
    def fetch_unread(self, max_results=10):
        ...
        resp = requests.get(
            ...
            headers=self._headers(),
            params={"labelIds": "UNREAD", "maxResults": max_results, "q": "in:inbox is:unread"},
            timeout=self.timeout,
        )
        if resp.status_code != 200:
        ...
        for ref in message_refs:
            ...
            signals.append({
                ...
                "metadata": {
                    ...
                    # Horodatage réel Gmail (ms epoch, toujours présent quel que
                    # soit le format demandé) — utile pour le diagnostic
                    # d'ordre de récupération, sans impact sur le comportement
                    # existant (champ purement informatif).
